...
def show_workshop_monitor(df_asistencia, centro_seleccionado):
    ...
    st.markdown("<h4 style='font-size:0.9rem; margin-bottom:10px; color:var(--text-secondary); text-transform:uppercase;'>Control de Carga (Hoy)</h4>", unsafe_allow_html=True)
...
def page_registrar_asistencia(df_personas, df_asistencia, centro, nombre_visible, usuario):
    ...
    notas = st.text_area("Notas generales del día (Opcional)", height=70, placeholder="Ej: Jornada tranquila.")
    ...
    presentes = st.multiselect("Marcar quiénes asistieron:", options=nombres, placeholder="Buscar y seleccionar...")
            ...
            try:
                ...
                if "23505" in err_str or "already exists" in err_str.lower():
                    ...
                    <div class='alert-box alert-warning'>
                        <b>Ya registrado hoy:</b> Ya se guardó la asistencia de '{espacio}' para la fecha elegida.<br><br>
...
def page_global(df_asistencia, df_personas):
    ...
    for c in CENTROS:
        ...
        if not faltan:
            st.markdown(f"✅ **{c}**: Todo cargado hoy.")
        else:
