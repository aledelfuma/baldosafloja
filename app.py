...
# ======================================================
# CONFIGURACIÓN DE TEMA CLARO PREMIUM (PURE GEOMETRY)
# ======================================================
st.set_page_config(
...
/* BARRA SUPERIOR GEOMÉTRICA LIMPIA */
.top-bar {
...
.user-info { font-size: 1.1rem; font-weight: 700; color: var(--text-primary); }
.center-info { font-size: 0.85rem; font-weight: 500; color: var(--text-secondary); margin-top: 2px; }

/* BOTONES PREMIUM MODO CLARO */
...
/* INPUTS E INTERFACES MODERNAS CON AIRE */
...
/* KPIs GEOMÉTRICOS CON SOMBRAS SUAVES */
.kpi {
  border-radius: var(--radius-lg);
  padding: 18px 14px;
...
.kpi h3 { margin: 0; font-size: 0.65rem; color: var(--text-secondary) !important; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600; }
.kpi .v { font-size: 1.9rem; font-weight: 800; color: var(--primary) !important; line-height: 1; margin-top: 8px; }

.alert-box { padding: 12px 15px; border-radius: var(--radius-sm); margin-bottom: 12px; font-size: 0.9rem; font-weight: 600; text-align: center; }
.alert-danger { background-color: #FEF2F2; color: #991B1B !important; border: 1px solid #FCA5A5; }
.alert-success { background-color: #F0FDF4; color: #166534 !important; border: 1px solid #BBF7D0; }
.alert-warning { background-color: #FFFBEB; color: #92400E !important; border: 1px solid #FDE047; }
.alert-gray { background-color: var(--surface); color: var(--text-secondary) !important; border: 1px solid var(--border-subtle); }

/* CONTROLADOR DE ACTIVIDADES (CALENDARIO SEMANAL) */
...
/* FICHA DE LEGAJO MINIMALISTA AJUSTADA */
.profile-card {
    background-color: var(--surface);
    border-radius: var(--radius-lg);
    padding: 24px;
...
.profile-institution { font-size: 0.65rem; font-weight: 700; letter-spacing: 0.05em; color: var(--primary); text-transform: uppercase; }
.profile-status { font-size: 0.75rem; font-weight: 600; }
.status-active { color: #166534; }
.status-inactive { color: #991B1B; }
.profile-name { font-size: 1.45rem; font-weight: 800; line-height: 1.1; margin-top: 4px; color: var(--text-primary); }
...
.profile-footer-data { background-color: #F8FAFC; padding: 14px; border-radius: var(--radius-sm); border: 1px solid var(--border-subtle); }

/* MENÚ FLOTANTE ELEVADO - TRASLÚCIDO CLARO PURE */
...
.stTabs [data-baseweb="tab"] {
    ...
    font-weight: 700;
    color: var(--text-secondary) !important; padding: 12px 0; 
...
.btn-wa {
    display: block; text-align: center; background-color: #16A34A !important; color: white !important;
    padding: 11px; border-radius: var(--radius-sm); text-decoration: none; font-weight: 700; font-size: 0.9rem; margin-top: 12px;
...
# ======================================================
# CONFIGURACIÓN DEL CALENDARIO DIARIO
# ======================================================
C_BELEN = "Calle Belén"
C_NUDO = "Nudo a Nudo"
...
def show_login_screen():
    ...
    st.markdown("""
    <div style='text-align: center; margin-top: 60px; font-size: 0.85rem; color: var(--text-secondary);'>
        Hogar de Cristo Bahía Blanca <br>
        <a href='mailto:alejandrodelfuma@gmail.com' style='color: var(--primary); text-decoration: none; font-weight: 600;'>
            Soporte Técnico
        </a>
    </div>
...
def show_workshop_monitor(df_asistencia, centro_seleccionado, fecha_seleccionada):
    ...
    for act in actividades_del_dia:
        if act in actividades_cargadas:
            html_monitor += f"<div class='workshop-row'><span class='workshop-name'>• {act}</span><span class='workshop-badge badge-done'>Cargado</span></div>"
        else:
            html_monitor += f"<div class='workshop-row'><span class='workshop-name'>• {act}</span><span class='workshop-badge badge-pending'>Falta Cargar</span></div>"
    html_monitor += "</div>"
...
def kpi_row_full(df_asistencia, centro):
    ...
    if not df_asistencia.empty:
        ...
        df_centro = df_kpi if centro in ["Administración", "coordinacion"] else df_kpi[df_kpi["centro"] == centro]
...
# ======================================================
# PESTAÑAS OPERATIVAS FORMULARIOS
# ======================================================
def page_registrar_asistencia(df_personas, df_asistencia, centro, nombre_visible, usuario):
    ...
    centro_seleccionado = st.selectbox("Seleccionar Centro a gestionar:", CENTROS) if centro in ["Administración", "coordinacion"] else centro
    ...
    if st.button("GUARDAR ASISTENCIA (SUPABASE)", type="primary", use_container_width=True):
        ...
        with st.spinner("Procesando en Supabase..."):
            try:
                ...
                st.balloons(); st.toast("Cambios guardados correctamente"); time.sleep(1); st.rerun()
                ...
                if "23505" in err_str or "already exists" in err_str.lower():
                    ...
                    """, unsafe_allow_html=True)
                else: st.error(f"Error inesperado: {e}")
...
def page_personas_full(df_personas, df_ap, df_seg, centro, usuario):
    ...
    centro_seleccionado = st.selectbox("Filtrar padrón por centro barrial:", CENTROS, key="padrón_admin_select") if centro in ["Administración", "coordinacion"] else centro
    df_centro = filter_personas_centro(df_personas, centro_seleccionado)
    ...
    if not seleccion:
        ...
        if not df_centro.empty:
            ...
            df_mostrar = df_centro.copy()
            if filtro_activo == "Solo Activos":
                df_mostrar = df_mostrar[df_mostrar["activo"].astype(str).str.upper() == "SI"]
    ...
    dni_val = "S/D" if (not str(datos_persona.get('dni')).strip() or str(datos_persona.get('dni')).lower() == 'none') else str(datos_persona.get('dni')).strip()
    ...
    direccion_mostrar = "No registrada" if (not str(datos_persona.get('domicilio')).strip() or str(datos_persona.get('domicilio')).lower() == 'none') else str(datos_persona.get('domicilio')).strip()
    ...
    <div class="profile-card">
        <div class="profile-header">
            ...
            <span class="profile-status {'status-active' if is_active else 'status-inactive'}">{"• Activo" if is_active else "• Inactivo"}</span>
        </div>
        <div class="profile-grid">
            <div class="profile-meta-item"><span class="profile-meta-label">Documento</span><span class="profile-meta-value">{dni_val}</span></div>
            <div class="profile-meta-item"><span class="profile-meta-label">Nacimiento / Edad</span><span class="profile-meta-value">{nacimiento_mostrar}</span></div>
            <div class="profile-meta-item" style="grid-column: span 2;"><span class="profile-meta-label">Direccion</span><span class="profile-meta-value">{direccion_mostrar}</span></div>
        </div>
    """, unsafe_allow_html=True)
    
    if tags_str and tags_str.lower() not in ["none", "nan"]:
        st.markdown(f"<div class='profile-footer-data'><span class='profile-meta-label'>Referencia Familiar</span><div>{tags_str}</div></div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    with st.form("form_bitacora_seguimiento", clear_on_submit=True):
        ...
        obs_nota = st.text_area("¿Qué pasó hoy?")
        
        if st.form_submit_button("Guardar en Bitácora (Supabase)", use_container_width=True):
            if not obs_nota.strip(): st.error("La observación no puede quedar vacía.")
            else:
                with st.spinner("Guardando..."):
                    try:
                        st.cache_data.clear()
                        supabase.table("bitacora_seguimiento").insert({
                            "fecha": f_nota.isoformat(), "anio": year_of(f_nota.isoformat()), "centro": centro_seleccionado,
                            "nombre_persona": seleccion, "categoria": cat_nota, "observacion": obs_nota.strip(), "usuario_registro": usuario
                        }).execute()
                        st.toast("Nota registrada"); time.sleep(0.5); st.rerun()
    ...
    if not df_chico.empty:
        for _, row in df_chico.sort_values("fecha", ascending=False).iterrows():
            st.markdown(f"""
            <div class='note-card'>
                <div style='font-size:0.75rem; color:var(--text-secondary); margin-bottom:4px;'><b>{row['fecha']}</b> — {row['categoria']} (Por: {row['usuario_registro']})</div>
                <div style='font-size:0.95rem; line-height:1.4;'>{row['observacion']}</div>
            </div>
            """, unsafe_allow_html=True)
...
def page_alta_persona(df_personas, centro, usuario):
    ...
    with st.form("alta_directa_form"):
        ...
        nac = st.text_input("Fecha de Nacimiento (AAAA-MM-DD)")
        tel = st.text_input("Teléfono")
        em = st.text_input("Contacto de Emergencia")
        dom = st.text_input("Dirección / Barrio")
        etq = st.text_input("Etiquetas / Referencias")
        not_p = st.text_area("Notas Permanentes")
        
        if st.form_submit_button("Guardar en el Padrón (Supabase)", type="primary", use_container_width=True):
            if not nom.strip(): st.error("El Nombre Completo es obligatorio.")
            else:
                with st.spinner("Guardando..."):
                    try:
                        st.cache_data.clear()
                        check = supabase.table("personas").select("*").eq("centro", centro_destino).ilike("nombre", nom.strip()).execute()
                        if check.data: st.warning(f"'{nom}' ya existe en este centro.")
                        else:
                            fecha_nac_final = pd.to_datetime(nac.strip()).date().isoformat() if nac.strip() else None
                            supabase.table("personas").insert({
                                "nombre": nom.strip(), "dni": dni.strip() if dni.strip() else None,
                                "fecha_nacimiento": fecha_nac_final, "telefono": tel.strip() if tel.strip() else None,
                                "domicilio": dom.strip() if dom.strip() else None, "contacto_emergencia": em.strip() if em.strip() else None,
                                "etiquetas": etq.strip() if etq.strip() else None, "notas": not_p.strip() if not_p.strip() else None,
                                "activo": "SI", "centro": centro_destino, "usuario_alta": usuario
                            }).execute()
                            st.balloons(); st.success("¡Ingresado correctamente!"); time.sleep(1); st.rerun()
                    except Exception as e: st.error(f"Error: {e}")
...
def page_reportes(df_asistencia, centro):
    ...
    if df_c.empty:
        st.markdown("<div class='alert-box alert-gray'>No hay datos históricos suficientes.</div>", unsafe_allow_html=True)
        return
    ...
    with m1:
        ...
        <div class='kpi'>
            ...
            <div class='v'>{sum_sem_actual} <span style='font-size:0.9rem; color:{c_wow}; font-weight:700;'>({wow_pct:+.1f}%)</span></div>
        </div>
        """, unsafe_allow_html=True)
    with m2:
        ...
        <div class='kpi'>
            ...
            <div class='v'>{sum_mes_actual} <span style='font-size:0.9rem; color:{c_mom}; font-weight:700;'>({mom_pct:+.1f}%)</span></div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("<br>#### Evolución Lineal de Concurrencia", unsafe_allow_html=True)
    st.line_chart(df_c.groupby("fecha")["presentes_i"].sum(), color="#2563EB")
